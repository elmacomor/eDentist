...
class OrderDetailsScreen extends StatefulWidget {
  ...
  const OrderDetailsScreen({Key? key, required this.narudzba})
      : super(key: key);
...
class _OrderDetailsScreenState extends State<OrderDetailsScreen> {
  ...
  void _updateOrderStatus() async {
    try {
      OrderHeader request = new OrderHeader(
          widget.narudzba.orderHeaderId,
          widget.narudzba.orderDate,
          widget.narudzba.userId,
          widget.narudzba.totalPrice,
          "closed");
      await _orderProvider.update(widget.narudzba.orderHeaderId, request);
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Narudžba je zatvorena.'),
      ...
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(
          builder: (BuildContext context) => OrdersListScreen(),
        ),
      );
    } catch (e) {
      print(e);
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Greška prilikom zatvaranja narudžbe.'),
          duration: Duration(seconds: 2),
        ),
      );
    }
